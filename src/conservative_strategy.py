...
for i in range(window_size, len(combined)):
    ...
    try:
        ...
        state_mean = grouped.mean()
        state_std = grouped.std()
        state_sharpe = state_mean / state_std
        high_risk_state = state_sharpe.idxmin()

        weight_gld = pi_next[high_risk_state]
        weight_spy = 1 - weight_gld

        #SOFT filter: high volatility --> SPY max 70%
        recent_vol = train_data['Volatility'].iloc[-1]
        if recent_vol > 0.025:
            weight_spy = min(weight_spy, 0.7)
            weight_gld = 1 - weight_spy

        # HARD filter: 12-week drawdown < -15% --> SPY = 0
        recent_cum_returns = (1 + combined['SPY_Return'].iloc[i-12:i]).cumprod()
        recent_dd = recent_cum_returns / recent_cum_returns.cummax() - 1
        if recent_dd.min() < -0.15:
            weight_spy = 0.0
            weight_gld = 1.0
    ...
    portfolio_growth = weight_spy * np.exp(r_spy) + weight_gld * np.exp(r_gld)
    portfolio_growth = max(portfolio_growth, 1e-8)
    strategy_return = np.log(portfolio_growth)
...
plt.title("Walk-Forward Gaussian HMM Strategy with Risk Filters vs Benchmarks [conservative_strategy]")
